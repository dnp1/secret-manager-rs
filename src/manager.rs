...
mod tests {
    ...
    impl SecretRotationBackend for MockBackend {
        ...
        async fn try_insert_key(&self, _group_id: Uuid, _expected_version: Option<u8>, _new_version: u8, _key_bytes: &[u8], _activated_at: SystemTime) -> Result<bool, MockError> {
            Ok(false)
        }
    }
    ...
    async fn start_hydrates_group_and_returns_ok() {
        let backend = MockBackend {
            load_response: vec![KeyRecord { id: 1, version: 0, key_bytes: vec![0xAA; 32], activated_at: SystemTime::now() - Duration::from_secs(300) }],
        ...
        let manager = SecretManager::new(Uuid::nil(), Arc::clone(&group), backend, Duration::from_secs(3600), Duration::from_secs(10), None, || [0xFFu8; 32]);
        manager.start(CancellationToken::new()).await.expect("start should succeed");
